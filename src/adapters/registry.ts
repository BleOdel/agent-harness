...
import { pythonPip } from "./python-pip.ts";
...
export function getAdapter(reference: Reference): ProjectAdapter {
 if (reference.id === pythonPip.reference.id && reference.version === pythonPip.reference.version) return pythonPip;
 if (reference.id === nodeNpm.reference.id && reference.version === nodeNpm.reference.version) return nodeNpm;
 throw new OperatorError(`Unsupported adapter ${reference.id}@${reference.version}. This release supports node-npm@1 and python-pip@1. Use harness project setup.`);
}
...
export async function detectProjects(project: string): Promise<string[]> {
 const definitions = [{ id: "node-npm", markers: nodeNpm.markers }, { id: "python-pip", markers: [...pythonPip.markers, "requirements.txt"] }, { id: "rust", markers: ["Cargo.toml"] }, { id: "go", markers: ["go.mod"] }];
 const found: string[] = [];
