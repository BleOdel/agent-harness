...
export async function writeServerRuntime(directory:string):Promise<void>{
 await mkdir(directory,{recursive:true,mode:0o700});
 await writeFile(path.join(directory,'server.mjs'),source,{mode:0o444,flag:'wx'});
}
export function serverRuntimePrompt():string {
 return `For Node server probes use the harness-owned helper, not generated child-process lifecycle code. Import {withServer} from '${SERVER_MODULE}' and set each importing step's serverRuntime to '${SERVER_DIGEST}'. This helper is mounted read-only during offline acceptance; never import a builder-authored helper. Legacy probes without imports remain supported.
...
export function serverRuntimeReview(steps:readonly CheckStep[]):string {
 if(!steps.some(s=>s.serverRuntime!==undefined||s.command.some(arg=>arg.includes(SERVER_MODULE)))) return '';
 return serverRuntimePrompt()+'\nHarness-owned exact helper source, SHA-256 '+SERVER_DIGEST+':\n'+source.toString('utf8');
}
