...
export async function prepareInParts(project:string,task:Feature,feedback:string,previous:Proposal|undefined,save:(state:Preparation)=>Promise<void>,progress:(text:string)=>void,saved?:Preparation,previousIssues:readonly string[]=[]):Promise<Proposal>{
 return draftInParts(task,{
  ...
  generate:(id,b)=>requestCheckJson(project,[
   draftPrompt(task),
   JSON.stringify({previousReviewFindings:previousIssues}),
   'Generate ONLY the selected behaviour against the frozen contract below. Return one case object {id,tasks,description,steps}, not a proposal. Copy its id, task and description exactly. Omit unused optional fields: never emit empty stdoutIncludes or empty files arrays. Every step needs a non-empty observable output expectation or expected file. Do not include contract or approval metadata. At most 16 KiB of JSON-encoded steps, including helpers. Exercise the selected behaviour, not every criterion in the task. Reuse compact reliable request/start/stop helpers within this code; keep timeouts and cleanup. Do not add routes, product requirements or broad assertions for other behaviours. The expected output must describe observations, never a hardcoded pass flag.',
