...
import { assertSnapshot, captureBaseline, captureCandidate, type Candidate, type Snapshot } from "../workspace/candidate.ts";
...
import { freezeChecks } from "./checks.ts";
...
export async function driveTeam(directory: string, worker: Worker): Promise<TeamState> {
  ...
  try {
    while (state.status === "running") {
      ...
      if (!merged.ok) {
        state = await appendEvent(directory, { type: "failed", attemptId: id, reason: merged.reason });
        await cleanup(); continue;
      }
      ...
      if (!integration.passed) {
        state = await appendEvent(directory, { type: integration.blocked ? "blocked" : "failed", attemptId: id, reason: integration.reason ?? "Combined project verification failed." });
        await cleanup(); continue;
      }
