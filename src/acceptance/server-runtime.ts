import {assertHttpRuntimes,writeHttpRuntime,httpRuntimePrompt,httpRuntimeReview} from './http-runtime.ts';
...
export function assertServerRuntimes(manifest:CheckManifest):void {
 assertAssetRuntimes(manifest);assertHttpRuntimes(manifest);
...
export async function writeServerRuntime(directory:string,assets=false):Promise<void>{
 ...
 await writeHttpRuntime(directory);
