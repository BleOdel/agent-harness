...
test('guided desktop setup reviews default journey, requires approval and selects saved work by title',async t=>{
 const root=await mkdtemp(path.join(await realpath(tmpdir()),'desktop-guide-')),project=root+'/app';await mkdir(project);t.after(()=>rm(root,{recursive:true,force:true}));
...
test('desktop init creates usable files, refuses overwrite and does not add dependencies',async t=>{
 const root=await mkdtemp(path.join(await realpath(tmpdir()),'desktop-init-'));t.after(()=>rm(root,{recursive:true,force:true}));await init(root,['--desktop']);
