...
test('desktop approvals detect edits and reports bind to approved runtime/source',async t=>{
 const root=await mkdtemp(path.join(await realpath(tmpdir()),'desktop-state-')),project=root+'/app';await mkdir(project);t.after(()=>rm(root,{recursive:true,force:true}));
...
test('desktop outputs reject aliases, excessive bytes and malformed PNGs before retention',async t=>{
 const root=await mkdtemp(path.join(await realpath(tmpdir()),'desktop-output-'));t.after(()=>rm(root,{recursive:true,force:true}));await writeFile(root+'/small','data');assert.equal((await boundedOutput(root,'small',4)).toString(),'data');
