...
import {HTTP_DIGEST,HTTP_MODULE} from '../src/acceptance/http-runtime.ts';
...
test('Docker: pinned read-only helper cleans Linux process groups, persists restarts and rejects wrong application output',{skip:configured?false:'configure Docker for helper verification'},async()=>{
 ...
 const probe=`import assert from 'node:assert/strict';import {readFile,writeFile,access} from 'node:fs/promises';import {withServer} from '${SERVER_MODULE}';import {observeHttp} from '${HTTP_MODULE}';let directory,pid;const observed=[];await assert.rejects(writeFile('${SERVER_MODULE}','bad'));await assert.rejects(writeFile('${HTTP_MODULE}','bad'));await withServer({command:['node','app.js'],databaseEnv:'APP_DB',ready:/^Listening at (http:\\/\\/127\\.0\\.0\\.1:\\d+)$/,stopTimeoutMs:150},async app=>{directory=app.directory;observed.push((await observeHttp(app.url,{timeoutMs:2000})).text);await app.restart();observed.push((await observeHttp(app.url,{timeoutMs:2000})).text);pid=Number(await readFile('/work/child.pid','utf8'));});await assert.rejects(access(directory));let running=true;for(let i=0;i<40;i++){try{const state=(await readFile('/proc/'+pid+'/stat','utf8')).split(') ')[1][0];running=state!=='Z';}catch(e){if(e.code!=='ENOENT')throw e;running=false;}if(!running)break;await new Promise(r=>setTimeout(r,25));}assert.equal(running,false);console.log(observed.join(','));`;
 try{
  await writeFile(path.join(project,'package.json'),'{"type":"module"}');await writeFile(path.join(project,'app.js'),code);
  const input=path.join(root,'checks.json');await writeFile(input,JSON.stringify({version:1,cases:[{id:'server',tasks:['server'],steps:[{command:['node','--input-type=module','-e',probe],serverRuntime:SERVER_DIGEST,httpRuntime:HTTP_DIGEST,exitCode:0,stdout:'1,2\n'}]}]}));await approveChecks(project,input);
