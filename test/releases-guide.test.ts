...
test('release guide saves a reviewable draft, preserves declined approval and selects actions without IDs',async t=>{
 const root=await mkdtemp(path.join(await realpath(tmpdir()),'release-guide-')),project=root+'/app',out=root+'/out';await mkdir(project);await mkdir(out);t.after(()=>rm(root,{recursive:true,force:true}));
