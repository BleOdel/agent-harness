/** Byte-preserving observations. The legacy text-only runtime stays immutable for existing pins. */
export async function observeHttp(url, options = {}) {
 ...
 return {url:response.url,status:response.status,headers:Object.fromEntries(response.headers),bytes,text:bytes.toString('utf8')};
}
