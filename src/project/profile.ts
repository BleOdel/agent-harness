...
export function defaultProfile(): ProjectProfile { return { version: 1, adapter: { id: "node-npm", version: 1 }, runner: { id: "docker", version: 1 }, requirements: { os: "linux", arch: "any", cpu: 2, memoryMiB: 2048, gui: false, emulator: false, gpu: false, toolchains: { node: 26, npm: 10 } }, skills: { build: ["codebase-design", "diagnosing-bugs", "domain-modeling", "tdd"], plan: ["grill-me", "grilling", "domain-modeling", "codebase-design"] } }; }
...
export function parseProfile(raw: unknown): ProjectProfile {
 ...
 if ((toolchains.node ?? 0) < 26 || (toolchains.npm ?? 0) < 10) throw new OperatorError("Node adapter requirements need Node >=26 and npm >=10.");
...
export async function readProfile(project: string, allowEmpty = false): Promise<ProjectProfile> {
 ...
 throw new OperatorError(`No supported adapter selected${detected.length ? ` (${detected.join(", ")})` : ""}. Use harness project setup. This release builds single-package Node/npm projects.`);
}
